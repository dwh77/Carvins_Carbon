...
# write.csv(chla_ccr, "../Data/CCR_chla.csv", row.names = F)
...
##run segmented regression: get function from script 6
chla_chem_segment <- chla_chem |> filter(!is.na(Chla_ugL))

z <- run_breakpoint_analysis(chla_chem_segment, response_var = "Chla_ugL")
...
```{r}
hpb_stage_STAGED <- read_csv("https://pasta-s.lternet.edu/package/data/eml/edi/1781/2/630f42ffb3560c3a6afd592511756c1e")
...
daily_q <- hpb_stage_STAGED |> 
  mutate(Date = as.Date(DateTime)) |> 
  group_by(Date) |> 
  summarise(Q_daily_cms = mean(Flow_cms, na.rm = T))
...
```{r}
daily_q |> 
  ggplot(aes(x= Date, y = Q_daily_cms ))+
  geom_point()


full_join(daily_q, hpb_flowmate, by = "Date") |> 
  filter(Date > ymd("2024-04-30"),
         Date < ymd("2025-05-01")) |> 
  ggplot(aes(x = Date))+
  geom_point(aes( y = Q_daily_cms))+
  geom_point(aes(y = Flow_cms), size = 4, color = "red")+
  scale_y_log10()+
  theme_bw()
...
Stream temp from MSN

```{r}
hpb_stage_STAGED |> 
  mutate(Date = as.Date(DateTime)) |> 
  filter(Date >= ymd("2024-07-31"),
         Date <= ymd("2024-08-01")) |> 
