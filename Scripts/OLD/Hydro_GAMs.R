...
hydro_daily <- read.csv("./Data/Hydrology_Daily_Observed.csv") |> 
  ...
  filter(Date >= ymd("2024-05-01"),
         Date <= ymd("2025-04-30")) |> 
  mutate(HPB_daily_Stage_cm = ifelse(Date > ymd("2025-02-11"), NA, HPB_daily_Stage_cm),
         Daily_Q_cms = ifelse(Date > ymd("2025-02-11"), NA, Daily_Q_cms))
  

#### Gap fill Q 
#Catawba
hydro_daily |> 
  ggplot(aes(x = Flow_Catawba_cfs, y = Daily_Q_cms ))+
  geom_point()+
  stat_poly_line(method = "lm", linewidth = 2)+
  stat_poly_eq(formula=y~x, label.x = "left", label.y="top", parse=TRUE, inherit.aes = F,
               aes(x = Flow_Catawba_cfs , y = Daily_Q_cms , label=paste(..adj.rr.label..,..p.value.label..,sep="~~~"),size=3))+
  theme_bw()

#Tinker
hydro_daily |> 
  ggplot(aes(x = Stage_Tinker_ft , y = Daily_Q_cms ))+
  geom_point()+
  stat_poly_line(method = "lm", linewidth = 2)+
  stat_poly_eq(formula=y~x, label.x = "left", label.y="top", parse=TRUE, inherit.aes = F,
               aes(x = Stage_Tinker_ft , y = Daily_Q_cms , label=paste(..adj.rr.label..,..p.value.label..,sep="~~~"),size=3))+
  theme_bw()
...
##Tinker gam 
model <- gam(Daily_Q_cms ~ s(Flow_Tinker_cfs), data = hydro_daily) 
summary(model)

hydro_daily$predicted_hpb_T <- predict(model, newdata = hydro_daily)

##Catawba gam
model2 <- gam(Daily_Q_cms ~ s(Flow_Catawba_cfs), data = hydro_daily) 
summary(model2)

hydro_daily$predicted_hpb_C <- predict(model2, newdata = hydro_daily)

#Both USGS gam
model3 <- gam(Daily_Q_cms ~ s(Flow_Catawba_cfs) + s(Flow_Tinker_cfs), data = hydro_daily) 
summary(model3)
...
hydro_daily |> 
  ...
  select(Date, Daily_Q_cms, predicted_hpb_C, predicted_hpb_T, predicted_hpb_CT) |> 
  pivot_longer(-1) |> 
...
#look at residuals 
resid <- hydro_daily |> 
  mutate(resid_hpb_C = Daily_Q_cms - predicted_hpb_C,
         resid_hpb_T = Daily_Q_cms - predicted_hpb_T,
         resid_hpb_CT = Daily_Q_cms - predicted_hpb_CT)
...
##RMSE Calcs
sqrt(mean((hydro_daily$predicted_hpb_C - hydro_daily$Daily_Q_cms)^2, na.rm = T))
sqrt(mean((hydro_daily$predicted_hpb_T - hydro_daily$Daily_Q_cms)^2, na.rm = T))
sqrt(mean((hydro_daily$predicted_hpb_CT - hydro_daily$Daily_Q_cms)^2, na.rm = T))
