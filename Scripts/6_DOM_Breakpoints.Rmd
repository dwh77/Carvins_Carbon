...
BIX_breakpoint <- chem |> 
  ggplot(aes(x = Distance, y = BIX, fill = Depth_m))+
  geom_rect(aes(xmin = (my.lines - my.lines.SE), xmax = (my.lines + my.lines.SE), ymin = -Inf, ymax = Inf), 
            alpha = 0.1, fill = "gray", color = NA )+  ## add breakpoint line SE
  geom_point(shape = 21, size = 4) + 
  ...
  scale_x_continuous(breaks = c(0, 500, 1000, 1500))+ 
  scale_fill_gradient2(low = "red",  high ="blue", midpoint = 6,  guide = "colourbar", breaks = c(3,6,9, 15))+
  geom_vline(xintercept = my.lines, linetype = "dashed", size = 1.5) + ##add breakpoint line
  geom_line(data = dist_gradient, aes(x = Distance, y = fit), size = 1, inherit.aes = FALSE ) ## add breakpoint slopes
...
#### Surface v bottom breakpoints/abbreviated breakpoint code

Make a function to run these easier 
...
ggsave("../Figures/DOM_breakpoints_MS.png", Breakpoints_MS, height = 7, width = 7, units = "in")
...
THIS is the cleaned up workflow that went into the function above 
...
DOC_breakpoint <- chem |> 
  ...
  geom_point(shape = 21, size = 4) + 
  #  geom_jitter(shape = 21, size = 4, width = 10, height = 0)+
  theme_bw()+ theme(legend.position = "top", text = element_text(size = 18), panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  labs(x= "Distance from Stream (m)", y = "DOC (mg/L)", fill = "Depth (m)")+
  scale_x_continuous(breaks = c(0, 500, 1000, 1500))+ 
   scale_fill_gradient2(low = "red",  high ="blue", midpoint = 6,  guide = "colourbar", breaks = c(3,6,9, 15))+
  geom_vline(xintercept = my.lines, linetype = "dashed", size = 1.5) + ##add breakpoint line
